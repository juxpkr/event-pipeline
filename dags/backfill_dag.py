...
with DAG(
    ...
    **처리 기간**: {BACKFILL_START_DATE} ~ {BACKFILL_END_DATE}
    ...
    backfill_producer = BashOperator(
        task_id='backfill_producer_to_kafka',
        bash_command=f"PYTHONPATH={PROJECT_ROOT} python {PROJECT_ROOT}/src/processing/gdelt_producer_backfill.py --start-date '{BACKFILL_START_DATE}' --end-date '{BACKFILL_END_DATE}' --logical-date '{BACKFILL_START_DATE}'",
        env=dict(os.environ),
        doc_md="""
