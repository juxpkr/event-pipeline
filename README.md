...
**문제 정의**: 이 플랫폼의 대시보드는 두 가지 완전히 다른 목적의 쿼리를 동시에 처리해야 했습니다. 
1. **Overview 대시보드 (실시간성):** "최근 24시간 수집 현황"을 3초 이내로 조회해야 함. (`processed_at` 필터)
2. **Detail 대시보드 (분석 성능):** "2024년 1월 우크라이나"처럼 특정 과거 기간의 심층 분석이 필요함. (`priority_date` 필터)
...
**해결**: 이 딜레마를 해결하기 위해, Silver Layer에서 최종 데이터를 두 가지 목적의 테이블로 분기하고, 각각의 쿼리 패턴에 최적화된 파티션 전략을 적용하는 듀얼 파티셔닝 아키텍처를 설계했습니다.
...
2. #### `gdelt_events_detailed` (Detail 대시보드: I/O 최적화)
	- **전략:** `priority_date` (사건 발생 시간)를 기준으로 파티셔닝하여 "특정 기간" 분석 시 불필요한 I/O를 원천 차단했습니다.
	- **파티션 구조:** `s3a://warehouse/silver/gdelt_events_detailed/year=2024/month=01/`
	- **최적화된 쿼리:** "2024년 1월" 분석 쿼리 시, Spark는 `year=2024/month=01/` 파티션으로 즉시 점프합니다.
3. [핵심 로직] `priority_date` 5단계 Fallback (무결성 확보)
`priority_date` 파티셔닝의 최대 리스크는 키 값이 NULL이 되는 것입니다. NULL 키는 파티셔닝 실패 및 데이터 정합성 문제를 유발합니다.

이 문제를 해결하기 위해, 3개 소스(Events, Mentions, GKG)의 시간 정확도를 고려한 5단계 `F.coalesce()` Fallback 로직을 구현하여 100% 유효한 파티션 키를 보장했습니다.

**성과**: 이 듀얼 파티셔닝 설계를 통해, 실시간 대시보드와 심층 분석 대시보드 양쪽 모두의 성능 요구사항을 만족시켰습니다.
