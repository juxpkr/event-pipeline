...
Azure 관리형 서비스 기반 프로젝트 경험을 통해 데이터 파이프라인의 `what`을 만드는 전체 흐름을 직접 설계하며 경험했습니다. 하지만 저는 그 편리한 블랙박스 뒤에서 일어나는 `how`와 `why`를 알고 싶었고, 클라우드 벤더에 종속되지 않는 기술력을 확보하기 위해, 오픈소스 스택을 기반으로 데이터 플랫폼을 처음부터 직접 구축하게 되었습니다.
...
- **분석**: 하지만 이 아키텍처는 `dbt-spark` 어댑터가 요구하는 Hive Metastore와, Spark가 내장하고 있는 Hive 버전이 충돌하며 수많은 `ClassNotFoundException`을 발생시켰습니다. 처음에는 공식 문서를 기반으로 `Dockerfile`의 JAR 파일을 직접 교체하거나, `spark-defaults.conf`를 수정하는 등 40개가 넘는 커밋을 통해 문제를 해결하려 했습니다. 그러나 모든 시도가 실패하며, 문제의 원인이 단순 JAR 파일이 아닌, 시스템의 설정 계층 전체에 걸쳐 있음을 확인했습니다.
- **해결:** 저는 근본 원인을 찾기 위해 Spark의 클래스패스 로딩 메커니즘을 분석했습니다. 그 결과, `Dockerfile`부터 `spark-defaults.conf`, 그리고 최종적으로 `docker-compose.yaml`의 `SPARK_OPTS` 변수까지 이어지는 여러 설정 계층 간의 충돌이 원인이었습니다. 특정 설정이 Spark 내장 Hive 라이브러리 사용을 강제하고 있었던 것입니다. 이에 공식 문서의 가이드를 따르기보다, Spark 내부의 자체적인 클래스패스 해석 로직을 신뢰하기로 결정하고, 충돌을 일으키는 명시적 버전 지정 관련 설정을 모두 제거했습니다.
...
- **데이터 기반 검증 및 문제 발견**: 저는 가설에만 의존하지 않고, 데이터로 검증하는 것을 최우선 과제로 삼았습니다.
	- **(가설 검증)** `4.6.2`에서 설명할 데이터 파이프라인 모니터링 시스템을 활용하여, 파이프라인 안정화 이후 축적된 실제 운영 데이터를 분석했습니다.
	- **(데이터 기반 분석)** 그 결과, 초기 가설과는 다르게 99.6%의 데이터가 단일 배치 내에서 즉시 조인된다는 사실을 데이터로 확인했습니다.
	- **(아키텍처 개선)** 이 분석 결과는, 단 0.4%의 예외 케이스(조인 실패)를 위해 15시간의 데이터를 매번 스캔하는 룩백 조인이 심각한 I/O 비효율을 초래하는 오버 엔지니어링임을 확인했습니다. 저는 이 데이터에 근거하여, 이 복잡한 룩백 조인 로직을 제거하고 `Left Join`으로 대체했습니다.
- **성과:** 데이터에 기반하여 불필요한 룩백 조인 로직을 제거함으로써, I/O 효율성을 확보하고 파이프라인의 복잡성을 크게 낮췄습니다. 이 경험을 통해, 초기 가설에 기반한 설계보다, 관측 가능성(Observability)을 먼저 확보하고 실제 데이터로 설계를 검증 및 개선하는 엔지니어링 접근 방식의 중요성을 깊이 이해하게 되었습니다.

#### 4.5 장애 시나리오 분리 기반 데이터 무결성 아키텍처 설계
- **문제 정의:** 안정적인 데이터 플랫폼은 예측 가능한 일시적 장애(네트워크 이슈 등)와 예측 불가능한 논리적 버그(코드 결함) 라는, 서로 다른 두 가지 성격의 장애가 복합적으로 발생하는 실제 운영 환경에서, 데이터 유실/중복/오염을 원천 차단하고 데이터 정합성(Consistency)을 확보해야 하는 문제가 있었습니다.

- **설계 및 해결:** 이 문제를 해결하기 위해, 저는 장애의 성격을 분리하고, 각 시나리오에 대응하는 다계층 방어 아키텍처를 설계했습니다.
	1. **일시적 장애 대응(안전한 자동 재시도 보장):**  일시적 장애는 자동 복구되어야 하지만, 재시도는 데이터 중복이나 불일치가 발생할 수 있습니다. 저는 이 재시도가 안전하도록 보장하는 데 집중했습니다.
		- **실행 무결성 (멱등성 + 원자성):** `data_interval_start`를 명시적으로 전달하여 100번을 재실행해도 항상 동일한 시간 구간만 처리하도록 멱등성을 보장했습니다. 또한, 3개의 스트림(Events, Mentions, GKG) 중 하나라도 실패 시, `gdelt_bronze_consumer.py`의 `q.stop()` 로직이 나머지 스트림을 즉시 중단시켰습니다. 이는 부분 성공으로 인한 데이터 불일치를 원천 차단하고 파이프라인의 원자성을 확보하는 핵심 설계였습니다.
  
		- **재시도 무결성 (다계층 중복 방어):** 재시도로 인해 유입되는 중복 데이터를 방어하기 위해 2중 방어 로직을 설계했습니다.
			 1. **소스 레벨:** `Window 함수` 를 사용해, `MERGE` 실행 전에 소스 데이터 자체의 중복을 1차 제거했습니다. (MERGE 오류 방지)
			2. **타겟 레벨:** `Delta Lake MERGE (UPSERT)` 기능을 통해, MERGE 실행중에 타겟 테이블의 중복을 감지하고 `INSERT` 대신 `UPDATE`를 실행하여 2차 방어했습니다.

	2. **논리적 버그 대응(데이터 오염 전파 차단):** 이 설계의 핵심은 논리적 버그를 일시적 장애와 다르게 취급하는 장애 격리입니다. 논리적 버그가 발생했을 때 자동 복구는 데이터를 오염시키므로, 정합성을 최우선으로 했습니다.
		- **데이터 품질(DQ) 검증:** `NULL`/`EMPTY` 키 등 무결성을 위반하는 결함 데이터를 파이프라인 유입 단계에서 사전 필터링했습니다.
		- **오염 전파 방지:** 3-Way Join 등 복잡한 로직이 포함된 `Silver Layer`(`gdelt_silver_processor.py`) 에서 코드 결함 발생 시, `try/except` 구문이 이를 감지하여 파이프라인이 즉시 중단되도록 설계했습니다. 이후, Airflow DAG의 의존성(`silver_processor >> trigger_dbt_gold_pipeline`) 을 통해, 이 중단이 상위 레이어인 Gold Layer(dbt)로 트리거되지 않도록 하여 데이터 오염 전파를  방지했습니다.
- **결과**: 이 설계를 통해, 일시적 장애는 자동으로 안전하게 복구하고, 논리적 버그는 즉시 격리하는 아키텍처를 구축했습니다. 어떤 유형의 장애나 재시도가 발생하더라도 최종 데이터 상태가 동일하게 보장되는 파이프라인 멱등성을 확보했으며, 논리적 버그 발생 시 오염된 데이터의 전파를 막아 플랫폼 전체의 데이터 정합성을 확보했습니다.

#### 4.6 관측 가능성(Observability) 확보를 위한 모니터링 시스템 구축
저는 파이프라인의 모든 동작을 투명하게 관측하고, 장애의 근본 원인을 파악하기 위해 애플리케이션 레벨과 데이터 레벨의 두 개의 모니터링 시스템을 직접 설계하고 구축했습니다.
#### 4.6.1 애플리케이션 레벨: Spark Custom Exporter 개발
...
#### 4.6.2 데이터 레벨: End-to-End Lifecycle Audit 시스템 구축
...
#### 4.7 분산 클러스터 배포 안정화 (네트워크, 의존성, 권한)
...
#### 4.8 쿼리 목적에 따른 듀얼 파티셔닝 아키텍처 설계
**문제 정의**: 이 플랫폼의 대시보드는 두 가지 다른 목적의 쿼리를 동시에 처리해야 했습니다. 
1. **Overview 대시보드 (실시간성):** "최근 24시간 수집 현황"을 빠르게 조회 (`processed_at` 필터)
2. **Detail 대시보드 (분석 성능):** "2024년 1월 우크라이나"처럼 특정 과거 기간의 상세 분석 (`priority_date` 필터)
단일 파티션 전략(`processed_at` 또는 `priority_date` 중 하나만 사용)으로는 이 상반된 쿼리 패턴을 동시에 최적화할 수 없었습니다. 한쪽은 빨라지지만, 다른 한쪽은 테이블 전체를 스캔하는 비효율이 발생했습니다.

**해결**: 이 비효율을 해결하기 위해, Silver Layer에서 최종 데이터를 두 가지 목적의 테이블로 분리하고, 각각의 쿼리 패턴에 최적화된 파티션 전략을 적용하는 듀얼 파티셔닝 아키텍처를 설계했습니다.
1. `gdelt_events` (Overview 대시보드: 실시간성 확보)
	- 전략: `processed_at` (수집 시간)을 기준으로 파티셔닝하여 최근 데이터 쿼리 성능을 극대화했습니다.
	- **파티션 구조:** `s3a://warehouse/silver/gdelt_events/year=2025/month=09/day=22/hour=14/`
	- **최적화된 쿼리:** "최근 24시간 수집 현황" 쿼리 시, Spark는 전체 수천 개의 파티션 중 정확히 최근 24개(`hour=XX`)의 파티션만 스캔합니다. 

2. `gdelt_events_detailed` (Detail 대시보드: I/O 최적화)
	- **전략:** `priority_date` (사건 발생 시간)를 기준으로 파티셔닝하여 특정 기간 분석 시 불필요한 I/O를 감소시켰습니다.
	- **파티션 구조:** `s3a://warehouse/silver/gdelt_events_detailed/year=2024/month=01/`
...
- Detail 쿼리: year=2024/month=01/ 직접 접근으로 불필요한 수백~수천 개 파티션 스킵
  
쿼리 패턴이 파티셔닝 전략을 결정하며, 상반된 쿼리 패턴은 단일 테이블로 동시에 최적화할 수 없다는 것을 이해하였고, 데이터의 사용 목적에 따라 파티셔닝 전략을 다르게 설계해야 함을 실전에서 경험했습니다.

#### 4.9 배포 자동화 스크립트 개발(`remote-deploy.sh`)
...
`섹션 4`에서 언급했듯이, 프로젝트 안정화 초기에는 `4.1`,`4.2`,`4.3`의 클러스터 문제(커넥션 누수, 의존성 충돌 등)와 `4.4`의 비효율적인 조인 로직 등 복합적인 문제들이 동시에 발생했습니다.
...
2. **CI/CD 파이프라인 자동화:** `4.9`에서 수동으로 실행하도록 개발한 배포 스크립트(`remote-deploy.sh`)를 GitHub Actions 기반으로 자동화합니다. Git Push 시 자동으로 빌드, 테스트, 배포가 이루어지는 CI/CD 파이프라인을 구축하여, 휴먼 에러를 방지하고 개발 생산성을 높일 것입니다.
