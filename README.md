...
- **데이터 기반 검증 및 문제 발견**: 저는 가설에만 의존하지 않고, 데이터로 검증하는 것을 최우선 과제로 삼았습니다.
	- **(관측 가능성 확보)** 가설 검증을 위해, `4.5.2`에서 설명할 데이터 파이프라인 모니터링 대시보드를 먼저 구축했습니다. 이 시스템은 데이터 레코드(`global_event_id` 기준)가 파이프라인을 통과하는 모든 단계의 상태와 타임스탬프를 추적했습니다
	- **(데이터 기반 분석)** 파이프라인 안정화 이후, 이 모니터링 시스템을 통해 축적된 실제 운영 데이터를 분석했습니다. 그 결과, 초기 가설과는 다르게 99.6%의 데이터가 단일 배치 내에서 즉시 조인된다는 사실을 데이터로 확인했습니다
